...
export default function CategoriesPage() {
  ...
  const save = async (event) => {
    ...
    if (!draft.name_vi.trim() || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(draft.slug)) {
      setStatus({ type: 'error', message: t('categories.validation') });
      return;
    }
    setSaving(true);
    const payload = {
    ...
    if (result.error) setStatus({ type: 'error', message: result.error.message });
      ...
      setStatus({ type: 'success', message: t('categories.saved') });
  ...
  const deactivate = async (category) => {
    ...
    const message = productCount
      ? t('categories.confirmReferenced', { count: productCount })
      : t('categories.confirmDeactivate');
  ...
  const saveImagePath = async (path) => {
    if (!editing) throw new Error(t('upload.saveFirst'));
  ...
  return (
    <AdminPage title={t('categories.title')} description={t('categories.description')}>
      <div className="admin-two-column">
        <section className="admin-card">
          <div className="admin-card__header"><h2>{t('categories.list')}</h2><button className="admin-button admin-button--primary" type="button" onClick={() => setEditing(null)}>{t('categories.add')}</button></div>
          {loading ? <AdminLoading /> : error ? <AdminError error={error} retry={load} /> : !categories.length ? <AdminEmpty title={t('categories.noData')} /> : (
            <div className="admin-list">
              {categories.map((category) => <button className={`admin-list-row${editing?.id === category.id ? ' is-active' : ''}`} type="button" key={category.id} onClick={() => setEditing(category)}>
                ...
                <span className={`admin-status admin-status--${category.ko_translation_status}`}>{t(`translation.${category.ko_translation_status || 'unknown'}`)}</span>
                <span className={category.active ? 'admin-chip admin-chip--success' : 'admin-chip'}>{t(category.active ? 'common.active' : 'common.inactive')}</span>
              </button>)}
            </div>
          )}
        </section>
        <form className="admin-card admin-form" onSubmit={save}>
          <h2>{editing ? t('categories.edit', { id: editing.id }) : t('categories.add')}</h2>
          ...
          <label className="admin-check"><input type="checkbox" checked={Boolean(draft.active)} onChange={(event) => change('active', event.target.checked)} /><span>{t('common.active')}</span></label>
          <ImageUploadField label={t('categories.image')} path={draft.image_path} folder={`categories/${editing?.id || 'new'}`} disabled={!editing} onUploaded={saveImagePath} />
          <SaveNotice status={status} />
          <div className="admin-inline-actions"><button className="admin-button admin-button--primary" type="submit" disabled={saving}>{saving ? t('common.saving') : t('categories.save')}</button>{editing?.active && <button className="admin-button admin-button--danger" type="button" onClick={() => deactivate(editing)}>{t('categories.deactivate')}</button>}</div>
        </form>
      </div>
    </AdminPage>
  );
